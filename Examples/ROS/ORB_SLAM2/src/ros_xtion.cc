...
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD, const sensor_msgs::JointStateConstPtr& j)
{
    ...
    //-----------------------------------------------------:
    Eigen::Isometry3f taratta;
    Eigen::Matrix<float,Eigen::Dynamic,Eigen::Dynamic> cose;
    cv::cv2eigen(t,cose);
    taratta.linear() = cose.block<3,3>(0,0);
    taratta.translation() = cose.block<3,1>(0,3);
    taratta = taratta.inverse();
    //ricordo di settare il baffodrago
    cv::Mat rgb = cv_ptrRGB->image.clone();
    cv::Mat d = cv_ptrD->image.clone();
    rgb_msg.setImage(rgb);
    rgb_msg.setOdometry(taratta);
    depth_msg.setImage(d);
    depth_msg.setOdometry(taratta);
    writer.writeMessage(rgb_msg);
    writer.writeMessage(depth_msg);
    writer.writeMessage(joint_msg);
    //    //Eigen::Vector3f lamerda = cose.block<3,1>(0,3);
    //    //Eigen::Matrix3f lamerdarota = cose.block<3,3>(0,0);

    //    Eigen::AngleAxisd rollAngle(0, Eigen::Vector3d::UnitZ());
    //    Eigen::AngleAxisd yawAngle(0, Eigen::Vector3d::UnitY());
    //    Eigen::AngleAxisd pitchAngle(180, Eigen::Vector3d::UnitX());
    //    Eigen::Quaternion<double> q = rollAngle * yawAngle * pitchAngle;
    //    Eigen::Matrix3d rotationMatrix = q.matrix();
    //    Eigen::Isometry3d faicose;
    //    faicose.Identity();
    //    faicose.linear() = rotationMatrix;

    //    //taratta = faicose*taratta;


    static tf::TransformBroadcaster br;
    tf::Transform transform;
    tf::transformEigenToTF((Eigen::Isometry3d)taratta,transform);
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "camera_link", "camera_pose"));
    std::cout<<transform.inverse().getOrigin().x()<< " "<<transform.inverse().getOrigin().y()<<" "<<transform.inverse().getOrigin().z()<<std::endl;
}
